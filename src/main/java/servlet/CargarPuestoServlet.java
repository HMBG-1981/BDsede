...
@WebServlet("/CargarPuestoServlet")
@MultipartConfig
public class CargarPuestoServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (FileInputStream fis = new FileInputStream(new File(filePath));
            ...
            try (Connection con = DriverManager.getConnection(url, user, pass)) {
                ...
                ps.close();
                response.sendRedirect("registroPuesto.jsp?mensaje=Se Insertaron " + registrosInsertados + " registros");
            }

        } catch (Exception e) {
            e.printStackTrace();
            response.sendRedirect("registroPuesto.jsp?mensaje=Error al cargar Excel: " + e.getMessage());
        }
    }
